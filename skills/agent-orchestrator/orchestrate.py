...
def _validate_runtime_config() -> None:
    """Strict runtime config validation (raise on invalid config)."""
    openclaw_agent = os.getenv("ORCH_OPENCLAW_AGENT_ID", "").strip()
    if openclaw_agent and not os.getenv("OPENCLAW_API_BASE_URL", "").strip():
        raise RuntimeError(
            "Invalid config: ORCH_OPENCLAW_AGENT_ID is set but OPENCLAW_API_BASE_URL is missing."
        )
...
def orchestrate(goal: str, tasks_override: dict = None, notifier: AgentChannelNotifier = None) -> dict:
    ...
    try:
        ...
        if created_async_notifier and hasattr(notifier, "close"):
